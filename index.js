...
function customAssign(...objects) {
  if (objects.length <2){
    ...
    const objArr = {};
    Object.assign (objArr, ...objects) ;
    for (const key in objArr){
      if (typeof objArr[key] === 'number'){
        objArr[key]= objArr[key].toString();
      }
    }
    return objArr;
  } 
...
function customEntries(obj) {
  ...
  const resultObjEntries = Object.entries(obj);
  return resultObjEntries;
...
function customObjectFromEntries(entries) {   
    if (!Array.isArray(entries) || entries.length === 0) {
      return "Помилка: Вхідний аргумент має бути масивом і містити записи.";
    }
...
function extendAndFreezeObject(obj1, obj2) {
  ...
  console.log(obj1);
  Object.freeze(obj1);
   const result =Object.isFrozen(obj1);
   if( result === true){
    return "заморожений";
   }else{
    return "ERROR";
...
function checkAndPreventExtensions(obj) {
  ...
  const checkIsExtensible =  Object.isExtensible(obj);
  if (checkIsExtensible === true){
...
function convertArrayToObj(arr) {
  ...
  const resultToObj = Object.fromEntries(Object.entries(obj)) ;
  return resultToObj;
